...
# 定义管道，包含PCA降维和SVM分类器
pipe = Pipeline([
    ('pca', PCA(n_components=30)),  # 进一步降低PCA的n_components以加快计算
    ('svm', SVC())
])
...
param_dist = {
    ...
    'svm__C': expon(scale=1),
    'svm__gamma': ['scale']
}

# 使用随机搜索和交叉验证进行超参数优化（减少折数）
start_time = time.time()
random_search = RandomizedSearchCV(pipe, param_distributions=param_dist, n_iter=10, cv=3, scoring='accuracy', random_state=42)  # 使用3折交叉验证和10次迭代
